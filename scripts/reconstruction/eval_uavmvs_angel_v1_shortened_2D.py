...
SCENE_NAME = "angel"
SCENE_FOLDER = os.path.join(FF_PROJECT_ROOT, "data", "uavmvs", "angel_v1")
INPUT_FOLDER = os.path.join(SCENE_FOLDER, "capture-shortened2D", "Reconstruction")
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(INPUT_FOLDER, "Evaluation") if args.out_dir is None else args.out_dir
    ...
    precision, recall, fscore, *histogram_data = TanksAndTemplesEvaluator.evaluate_reconstruction(
        ...
        gt_log_path=os.path.join(INPUT_FOLDER, "tanksandtemples.rec.log"),
        # Generated from Meshroom output with geometry_crop.py:
        est_ply_path=os.path.join(INPUT_FOLDER, "cropped_1.ply"),
        # Generated from Meshroom output with evaluate_reconstruction.py:
        est_log_path=os.path.join(INPUT_FOLDER, "tanksandtemples.sfm.log"),
        # Eyeballed in Meshlab from uavmvs and Meshroom outputs:
        align_txt_path=os.path.join(INPUT_FOLDER, "tanksandtemples.alignment.txt"),
        # Generated from Meshroom output with geometry_crop.py:
        crop_json_path=os.path.join(INPUT_FOLDER, "cropped_1.json"),
