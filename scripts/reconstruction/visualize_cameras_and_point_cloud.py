...
def visualize_cameras_and_point_cloud(
    ...
    if matrix_path is not None:
        ...
        o3dy.draw_source_aligned_to_target(
            source_cameras_pcd, target_cameras_pcd, transformation, source_scene_pcd, target_scene_pcd
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source_cameras_path", type=str, help="Path to the source camera point cloud PLY file"
    )
    parser.add_argument(
        "target_cameras_path", type=str, help="Path to the target camera point cloud PLY file"
    )
    parser.add_argument(
        "source_scene_path",
        type=str,
        help="Path to the source (reconstructed) scene point cloud PLY file",
    )
    parser.add_argument(
        "target_scene_path",
        type=str,
        help="Path to the target (ground-truth) scene point cloud PLY file",
    )
    parser.add_argument(
        "--source_to_target_path", "-s2t", type=str, help="Path to a alignment matrix TXT file"
    )
    parser.add_argument(
        "--flip_yz", "-yz", action="store_true", help="Flip Y and Z axis of target point cloud"
    )
