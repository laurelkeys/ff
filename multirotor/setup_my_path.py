...
class SetupPath:
    ...
    @staticmethod
    def getCurrentPath():
        cur_filepath = os.path.realpath(__file__) # __file__
        return os.path.dirname(cur_filepath)

    @staticmethod
    def getGreatGrandParentDir():
        cur_path = SetupPath.getCurrentPath()
        if SetupPath.getDirLevels(cur_path) >= 3:
...
if __name__ == "__main__":
    cur_path = SetupPath.getCurrentPath()
    print("getCurrentPath", cur_path)
    print("getDirLevels", SetupPath.getDirLevels(cur_path))
    print("getParentDir", SetupPath.getParentDir())
    print("getGrandParentDir", SetupPath.getGrandParentDir())
    print("getGreatGrandParentDir", SetupPath.getGreatGrandParentDir())