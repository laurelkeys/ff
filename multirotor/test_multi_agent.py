...
f1 = client.takeoffAsync(timeout_sec=10, vehicle_name="Drone1")
f2 = client.takeoffAsync(timeout_sec=10, vehicle_name="Drone2")
f1.join()
f2.join()
...
for drone_idx, (idx, response) in enumerate(enumerate(responses1 + responses2)):
    filename = os.path.join(tmp_dir, f"drone{drone_idx}-" + str(idx))
